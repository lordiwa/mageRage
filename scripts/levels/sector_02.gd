...
## TASK-045: this sector's camera clamp (playfield bounds). The shared player scene
## carries NO limits (so encounter/arena/test_level stay unclamped); each real sector
## sets them at runtime. Horizontal = corridor inner faces (WallLeft inner x=0, WallRight
## inner x=4600); vertical = the ceiling/floor body centers (y=-320 / y=+360), a band that
## keeps the view inside the widened backstop with no void at the camera extremes.
const CAM_LIMIT_LEFT := 0
const CAM_LIMIT_TOP := -320
const CAM_LIMIT_RIGHT := 4600
